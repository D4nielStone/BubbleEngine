...
namespace BubbleUI {
    ...
    // Construtor
    Painel::Painel(const bubble::vetor4<int>& rect) 
    {
        retangulo = (rect);
    }
    // Construtor
    Painel::Painel(const char* n, const bubble::vetor4<int>& rect) 
    {
        retangulo = (rect);
        Nome = n;
    }
    ...
    void Painel::definirContexto(std::shared_ptr<Contexto> ctx, const char* nome, const bubble::vetor4<int>& rect = { 0,0,0,0 })
    {
        Nome = nome;
        configurar(ctx, rect);
    }
    ...
    void Painel::defCor(const bubble::cor& cor)
    {
        Rect::defCor(cor);
    }

    // Configuração do Painel
    void Painel::configurar(std::shared_ptr<Contexto> ctx, const bubble::vetor4<int>& rect)
    {
        //retangulo = rect;
        contexto = ctx;
        widgetPadding = { 5, 5 };
        ...
        ponta_a = std::make_unique<Formas::MeioCirculo>(contexto, bubble::vetor4<int>{ 0, 0, espessuraBorda, espessuraBorda });
        ponta_b = std::make_unique<Formas::MeioCirculo>(contexto, bubble::vetor4<int>{ 0, 0, -espessuraBorda, espessuraBorda });
        ponta_c = std::make_unique<Formas::MeioCirculo>(contexto, bubble::vetor4<int>{ 0, 0, -espessuraBorda, -espessuraBorda });
        ponta_d = std::make_unique<Formas::MeioCirculo>(contexto, bubble::vetor4<int>{ 0, 0, espessuraBorda, -espessuraBorda });
