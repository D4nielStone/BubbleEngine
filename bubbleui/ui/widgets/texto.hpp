...
namespace BubbleUI
{
	namespace Widgets
	{
		...
		public:
			...
			bubble::vetor4<float> paraNDC(const bubble::vetor4<int>& vec4);
		...
		protected:
			bool desenharSelecao(Vector2 mouse_inicial, Vector2 mouse_final, bubble::vetor4<int> char_rect, size_t letra_idx);
			void renderizar_texto(std::string &frase);
			size_t texto_cursor_index{ 0 }; bubble::vetor4<float> texto_cursor_pos;
			...
			std::string frase;
			bubble::cor cor{1.f, 1.f, 1.f, 1.f}, cor_de_selecao{ 0.678F, 0.847F, 0.902F, 1.0F };
			Vector2 pos_texto;
			bubble::vetor4<int> char_rect, char_fundo_rect;
			std::vector<LetraRect> letras_rect;
			bubble::vetor4<int> area_de_selecao;
