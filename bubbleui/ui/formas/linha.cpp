...
Linha::Linha(const bubble::vetor4<int> &pos, std::shared_ptr<Contexto> ctx) : posicoes(pos), contexto(ctx)
{
...
void Linha::definirPosicao(const bubble::vetor4<int> &pos)
{
    posicoes = pos;
}
// Deve definir cor base
void Linha::defCor(const bubble::cor &cor)
{
    cor_base = cor;
}
void BubbleUI::Formas::Linha::definirCorRef(bubble::cor* cor)
{
    cor_referencial = cor;
}
// Deve transformar coordenadas pixel para NDC
bubble::vetor4<float> Linha::paraNDC()
{
