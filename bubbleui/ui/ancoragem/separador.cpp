...
{
	if (!colisao) colisao = std::make_shared < Colisao2d>(bubble::vetor4<int>(), contexto);
	atualizarColisao();
	atualizarArrasto();
	atualizarCursor();
}
...
void BubbleUI::Separador::atualizarColisao()
{
	...
	{
	case Horizontal:
		colisao->definirBounds(bubble::vetor4<int>( pai->bounds.x + pai->bounds.w * _Mproporcao - 5, pai->bounds.y, 5, pai->bounds.h  ));
		break;
	case Vertical:
		colisao->definirBounds(bubble::vetor4<int>(pai->bounds.x , pai->bounds.y + pai->bounds.h * _Mproporcao - 5, pai->bounds.w  , 5  ));
	default:
		break;
	}
}
