...
namespace BubbleUI
{
	namespace Widgets
	{
		...
		public:
			...
			Vector4<float> paraNDC(const Vector4<int>& vec4);
		...
		protected:
			bool desenharSelecao(Vector2 mouse_inicial, Vector2 mouse_final, Vector4<int> char_rect, size_t letra_idx);
			void renderizar_texto(std::string &frase);
			size_t texto_cursor_index{ 0 }; Vector4<float> texto_cursor_pos;
